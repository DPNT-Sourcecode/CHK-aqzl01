...
class CheckoutSolution {
    ...
    #skuCatalogue = new Set([...this.#buyNGetMSkus.keys(), ...this.#standardPricing.keys()])
    ...
    #pricingFactory({
        ...
        buyNGetMDeductionsHandler = undefined
    }) {
        return (itemCount, counts = null) => {
            if (itemCount <= 0) return 0; 

            if (buyNGetMDeductionsHandler && counts) {
                // for cross sku deals, modify the counts
                buyNGetMDeductionsHandler?.(itemCount)
                itemCount = counts[sku]
            }
    ...
    #getBuyNGetMDeals(counts) {
        return this.#buyNGetMSkus.entries().map(([boughtSku, deal]) => {
            const [receivedSku, buyN, _] = deal
            return [boughtSku, this.#pricingFactory({
                sku: boughtSku,
                buyNGetMDeductionsHandler: this.#deductions(counts, receivedSku, buyN)
            })]
        })
    }
