...
class CheckoutSolution {
    ...
    #applyGroupDiscounts(counts) {
        // pack as many discounts as possible, expensive first
        const eligibleItems = Object
            .entries(counts)
            .filter(([sku, _]) => this.#groupDiscountSkus.has(sku))
            .sort(([skuA], [skuB]) => 
                this.#standardPricing.get(skuA) - this.#standardPricing.get(skuB)
        );
        const queue = []
        for (const [sku, number] of eligibleItems) {
            const skuPrice = this.#standardPricing.get(sku)
            for (let i = 0; i < number; i++) {
                queue.push({sku, skuPrice})
            }
        }

        let total = 0
        let queuePointer = 0
        while (queuePointer + 2 < queue.length) {
            total += 45
            for (let groupIndex = 0; groupIndex < 3; groupIndex++) {
                // deduct 3 of most expensive in queue
                counts[queue[queuePointer + groupIndex].sku]--
            }
            queuePointer += 3
        }
        return total
    ...
    checkout(skus) {
        ...
        checkoutValue += this.#applyGroupDiscounts(counts)
