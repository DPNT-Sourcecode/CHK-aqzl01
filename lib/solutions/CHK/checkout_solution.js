...
class CheckoutSolution {
    ...
    #deductions(counts) {
        ...
        for (const [boughtSku, deal] of this.#buyNGetMSkus.entries()) {
            const buyN = deal[1]
            const applicableDeductions = Math.floor(counts[boughtSku] / buyN)
            if (applicableDeductions <= 0) continue
            counts[boughtSku] = Math.max(0, counts[boughtSku] - applicableDeductions)
        } 
    }
    ...
    #getBuyNGetMDeals() {
        this.#buyNGetMSkus.entries().map(([boughtSku, deal]) => {
            const [receivedSku, buyN, getM] = deal
